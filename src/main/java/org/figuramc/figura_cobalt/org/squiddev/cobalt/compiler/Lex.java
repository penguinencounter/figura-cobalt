...
import org.figuramc.figura_cobalt.cc.tweaked.cobalt.internal.string.CharProperties;
...
import org.figuramc.figura_cobalt.org.squiddev.cobalt.lib.Utf8Lib;
...
final class Lex {
	...
	private void next() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		current = z.read();
		columnNumber++;
	}
	...
	private void saveAndNext() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		save(current);
		next();
	}
	...
	Buffer createErrorMessage(int line) throws LuaUncatchableError {
		return new Buffer(state.allocationTracker).append(shortSource).append(":").append(Integer.toString(line)).append(": ");
	}

	CompileException lexError(String msg, int token) throws LuaUncatchableError {
	 ...
	 */
	LuaString newString(byte[] bytes, int offset, int len) throws LuaUncatchableError {
	...
	LuaString newString(String value) throws LuaUncatchableError {
	 ...
	 */
	private void inclineNumber() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		int old = current;
	...
	private boolean checkNext(char character) throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
	...
	private boolean checkNext(char c1, char c2) throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
	...
	private LuaNumber readNumeral() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
	 ...
	 */
	private int skipSep() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		int count = 0;
		int s = current;
		assert s == '[' || s == ']';
		saveAndNext();
	...
	private void readLongString(Token token, int sep) throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		int line = lineNumber;
	...
	private CompileException escapeError(String message) throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		if (current != EOZ) saveAndNext();
		return lexError(message, TK_STRING);
	}

	private int readHex() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		saveAndNext();
		if (!CharProperties.isHex(current)) throw escapeError("hexadecimal digit expected");
		return CharProperties.hexValue(current);
	}

	private int readHexEsc() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		int left = readHex();
		int right = readHex();
		bufferSize -= 2;
		return (left << 4) | right;
	...
	private void readUtf8Esc() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		saveAndNext();
	...
	private int readDecEsc() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		int i = 0;
		int result = 0;
	...
	private LuaString readString(int del) throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		saveAndNext();
		while (current != del) {
			switch (current) {
	...
	private void saveEscape(int character) throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		next();
		bufferSize--;
		save(character);
	}

	private int lexToken(Token token) throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		bufferSize = 0;
		while (true) {
	...
	void skipShebang() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
		if (current == '#') {
			while (!currIsNewline() && current != EOZ) next();
		}
	}

	void nextToken() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
	...
	Token lookahead() throws CompileException, LuaError, LuaUncatchableError, UnwindThrowable {
