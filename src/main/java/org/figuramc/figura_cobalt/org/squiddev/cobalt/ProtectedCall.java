...
import static org.figuramc.figura_cobalt.org.squiddev.cobalt.debug.DebugFrame.FLAG_ERROR;
...
public class ProtectedCall {
	 ...
	 */
	public Result apply(LuaState state, LuaValue func, Varargs args) throws UnwindThrowable, LuaUncatchableError {
	 ...
	 * @throws UnwindThrowable If the underlying function yields.
	 */
	public Result apply(LuaState state, SuspendedFunction<Varargs> task) throws UnwindThrowable, LuaUncatchableError {
	 ...
	 */
	public Result resume(LuaState state, Varargs args) throws UnwindThrowable, LuaUncatchableError {
		if (currentTask != null) {
			// If we're still inside our task, then resume it.
			try {
				return finishSuccess(state, currentTask.resume(args));
			} catch (LuaUncatchableError dontCatch) {
	 ...
	 */
	public Result resumeError(LuaState state, LuaError error) throws UnwindThrowable, LuaUncatchableError {
	...
	private Result callErrorHandler(LuaState state, Throwable error) throws UnwindThrowable, LuaUncatchableError {
	...
	private static class CallSuspended implements SuspendedFunction<Varargs> {
		...
		@Override
		public Varargs call(LuaState state) throws LuaError, LuaUncatchableError, UnwindThrowable {
