...
class StringPacker {
	...
	private final static class Buffer {
		...
		private Buffer(@Nullable AllocationTracker<LuaUncatchableError> allocTracker) {
			this.allocTracker = allocTracker;
		}

		void ensure(int bytes) throws LuaUncatchableError {
			if (output == null) {
	...
	private static class Info {
		...
		public Info(LuaString string, @Nullable AllocationTracker<LuaUncatchableError> allocTracker) {
			this.string = string;
			position = 0;
			end = string.length();
			this.allocTracker = allocTracker;
	...
	public static int getNumLimit(Info info, int def) throws LuaError, LuaUncatchableError {
	...
	public static Mode getOption(Info info) throws LuaError, LuaUncatchableError {
		byte c = info.string.byteAt(info.position++);
		return switch (c) {
	...
	public static Mode getDetails(Info info, int outPosition) throws LuaError, LuaUncatchableError {
	...
	private static void packInt(Buffer buffer, long num, boolean littleEndian, int size, boolean neg) throws LuaUncatchableError {
		buffer.ensure(size);
	 ...
	 */
	static LuaValue pack(LuaState state, Varargs args) throws LuaError, LuaUncatchableError {
	 ...
	 */
	static long packsize(LuaString fmt, @Nullable AllocationTracker<LuaUncatchableError> allocTracker) throws LuaError, LuaUncatchableError {
		int size = 0;
	...
	private static long unpackInt(LuaString str, int offset, boolean isLittle, int size, boolean signed, @Nullable AllocationTracker<LuaUncatchableError> allocTracker) throws LuaError, LuaUncatchableError {
		long res = 0;
	 ...
	 */
	static Varargs unpack(LuaState state, Varargs args) throws LuaError, LuaUncatchableError {
		LuaString fmt = args.arg(1).checkLuaString(state);
